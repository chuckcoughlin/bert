...
class StatementTranslator(bot: MessageBottle, val sharedDictionary: MutableMap<SharedKey, Any>) :
    ...
    override fun visitConfigurationQuestion(ctx: SpeechSyntaxParser.ConfigurationQuestionContext?): Any? {
        bottle.type = RequestType.GET_CONFIGURATION
        return null
    }

    // Get internal configuration parameters. There are no options.
    override fun visitConfigurationRequest(ctx: SpeechSyntaxParser.ConfigurationRequestContext?): Any? {
        bottle.type = RequestType.GET_CONFIGURATION
        return null
    }
