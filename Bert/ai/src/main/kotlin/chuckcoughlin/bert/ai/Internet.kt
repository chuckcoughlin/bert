...
/**
 * This is the class that handles user requests that don't match syntax known
 * to the robot. These are sent as-is to Chat GPT. The responses are forwarded
 * to the uer with no effect on control of the robot.
 *
 * This class routes requests/responses between the Dispatcher and the internet.
 * Communication with the Dispatcher is via Kotlin channels and communication with
 * the network is via HTTP.
 *
 * If there is no network connection configured, this class is not called
 * by the dispatcher.
 *
 * @param req - channel for internet requests as generated by the user
 * @param rsp - channel for responses from the internat (Dispatcher)
 */
class Internet(req : Channel<MessageBottle>,rsp: Channel<MessageBottle>) :Controller {
