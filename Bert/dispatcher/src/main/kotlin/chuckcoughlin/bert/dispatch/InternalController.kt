...
class InternalController(req: Channel<MessageBottle>,rsp: Channel<MessageBottle>) :
    ...
    // source will not produce a response to the original requestor.
    suspend private fun handleRequest(request: MessageBottle) {
        // Find limb for a single joint
        if( request.limb.equals(Limb.NONE) && !request.joint.equals(Joint.NONE) ) {
    ...
    // Duplicate the pose message for each limb
    suspend fun distributePose(request:MessageBottle) {
        for( limb in Limb.values()) {
            val msg = request.copy()
            msg.limb = limb
            if(!limb.equals(Limb.NONE)) msg.source = ControllerType.BITBUCKET
            dispatchMessage(msg)
        }
