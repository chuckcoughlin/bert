...
class InternalController(parent : Dispatcher,req: Channel<MessageBottle>,rsp: Channel<MessageBottle>) : Controller {
    ...
    override suspend fun execute() {
        if (!running) {
            ...
            /* Coroutine to process the next request from the queues
             */
            while (running) {
    ...
    private suspend fun handleRequest(msg: MessageBottle) {
        ...
        // New request to add to the queue
            ...
            // No limb, add directly to timer queue
            else {
                sendToTimerQueue(null, msg)
            }
        }
        job.cancelAndJoin()
    }
    ...
    suspend fun receiveResponse(msg: MessageBottle) {
        ...
        if (pendingMsg != null) {
            ...
            fromDispatcher.send(msg) // Will reply to the original source
        }
        else {
            LOGGER.info(
     ...
     */
    private fun sendToTimerQueue(queue: SequentialQueue?, msg: MessageBottle) {
        if (queue != null) {
            pendingMessages[index] = msg
    ...
    init {
        ...
        pendingMessages = HashMap<Long, MessageBottle>()
