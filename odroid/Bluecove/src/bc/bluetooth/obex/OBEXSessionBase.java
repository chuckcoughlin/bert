...
import bc.bluetooth.BluetoothConnectionAccess;
import bc.bluetooth.obex.OBEXAuthentication.Challenge;
...
import bc.javax.obex.Authenticator;
import bc.javax.obex.HeaderSet;
import bc.javax.obex.ServerRequestHandler;
...
abstract class OBEXSessionBase implements Connection, BluetoothConnectionAccess {
	private final static String CLSS = "OBEXSessionBase";
		...
		try {
			...
			if (!initOK) {
				try {
					...
					LOGGER.info(String.format("%s: close error (%s",CLSS,e.getLocalizedMessage()));
				}
			}
		}
	}
	...
	protected synchronized void writePacketWithFlags(int commId, byte[] headerFlagsData, OBEXHeaderSetImpl headers)
		...
		LOGGER.info(String.format("%s.writePacketWithFlags: obex sent (%d packets) %s",CLSS,this.packetsCountWrite, OBEXUtils.toStringObexResponseCodes(commId)));
		os.write(buf.toByteArray());
		os.flush();
		LOGGER.info(String.format("%s.writePacketWithFlags: obex sent (%d bytes)",CLSS, len));
	...
	protected synchronized byte[] readPacket() throws IOException {
		...
		LOGGER.info(String.format("%s.readPacket: obex received (%d packets) %s",CLSS,this.packetsCountRead, OBEXUtils.toStringObexResponseCodes(header[0])));
		int length = OBEXUtils.bytesToShort(header[1], header[2]);
		if (length == 3) {
			return header;
		}
		if ((length < 3) || (length > OBEXOperationCodes.OBEX_MAX_PACKET_LEN)) {
			throw new IOException("Invalid packet length " + length);
		}
		byte[] data = new byte[length];
		System.arraycopy(header, 0, data, 0, header.length);
		OBEXUtils.readFully(is, obexConnectionParams, data, header.length, length - header.length);
		if (is.available() > 0) {
			LOGGER.info(String.format("%s.readPacket: has more data after read: %s",CLSS,(is.available()>0?"true":"false")));
		}
		return data;
	}
