...
abstract class OBEXClientOperation implements Operation, OBEXOperation, OBEXOperationReceive, OBEXOperationDelivery {
	private final static String CLSS = "OBEXClientOperation";
	...
	public void deliverPacket(boolean finalPacket, byte[] buffer) throws IOException {
		...
		if (finalPacket) {
			...
			LOGGER.info(String.format("%s.deliverPacket:client Request Phase ended" , CLSS));
			requestEnded = true;
		}
	...
	protected void endRequestPhase() throws IOException {
		...
		LOGGER.info(String.format("%s.deliverPacket:client ends Request Phase" , CLSS));
		this.operationInProgress = false;
	...
	private void exchangePacket(OBEXHeaderSetImpl headers) throws IOException {
		...
		try {
			...
			LOGGER.info(String.format("%s.exchangePacket client operation got reply (%s)",CLSS, OBEXUtils.toStringObexResponseCodes(responseCode)));
			switch (responseCode) {
			case ResponseCodes.OBEX_HTTP_UNAUTHORIZED:
                if ((!authenticationResponseCreated) && (dataHeaders.hasAuthenticationChallenge())) {
                	LOGGER.info(String.format("%s.exchangePacket: client resend request with auth response" , CLSS));
...
//                }
	...
	protected void processIncommingData(HeaderSet dataHeaders, boolean eof) throws IOException {
		...
		if (data != null) {
			LOGGER.info(String.format("%s.processIncommingData: client received Data eof" , CLSS));
	...
	private void receiveOperationEnd() throws IOException {
		while (!isClosed() && (operationInContinue)) {
			LOGGER.info(String.format("%s.receiveOperationEnd: operation expects operation end" , CLSS));
	...
	public void close() throws IOException {
		try {
			...
			if (!this.isClosed) {
				this.isClosed = true;
				LOGGER.info(String.format("%s.close: client operation closed" , CLSS));
			}
		}
	}
