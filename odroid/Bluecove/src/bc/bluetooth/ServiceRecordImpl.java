...
public class ServiceRecordImpl implements ServiceRecord {
	private final static String CLSS = "ServiceRecordImpl";
	...
	public void loadByteArray(byte data[]) throws IOException {
	...
	public boolean populateRecord(int[] attrIDs) throws IOException {
		...
		for (int i = 0; i < sortIDs.length - 1; i++) {
			...
			LOGGER.info(String.format("%s.populateRecord: sortId %d", CLSS,sortIDs[i]));
		}
		LOGGER.info(String.format("%s.populateRecord:  query for attr %d ids", CLSS,sortIDs.length));
	...
	public String getConnectionURL(int requiredSecurity, boolean mustBeMaster) {
		...
		if (device == null) {
			try {
				...
				LOGGER.info(String.format("%s.getConnectionURL: can't read LocalAddress %s", CLSS,bse.getLocalizedMessage()));
	 ...
	 */
	public void populateAttributeValue(int attrID, DataElement attrValue) {
		if (attrID < 0x0000 || attrID > 0xffff) {
			throw new IllegalArgumentException();
		}
		if (attrValue == null) {
	...
	public boolean hasServiceClassUUID(UUID uuid) {
		...
		if ((value == null) || (!(value instanceof Enumeration))) {
			LOGGER.info(String.format("%s.hasServiceClassUUID: Bogus value in DATSEQ class", CLSS));
			if (value != null) {
				LOGGER.info(String.format("%s.hasServiceClassUUID: DATSEQ class (%s)", CLSS,value.getClass().getName()));
			}
			return false;
		}
		for (Enumeration e = (Enumeration) value; e.hasMoreElements();) {
			...
			if (!(element instanceof DataElement)) {
				LOGGER.info(String.format("%s.hasServiceClassUUID: Bogus element in DATSEQ (%s)", CLSS,value.getClass().getName()));
				continue;
			}
