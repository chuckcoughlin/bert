...
import bc.bluetooth.DeviceInquiryRunnable;
import bc.bluetooth.DeviceInquiryThread;
import bc.bluetooth.NotSupportedIOException;
import bc.bluetooth.NotSupportedRuntimeException;
import bc.bluetooth.RemoteDeviceHelper;
import bc.bluetooth.SearchServicesDeviceNotReachableException;
import bc.bluetooth.SearchServicesException;
import bc.bluetooth.SearchServicesRunnable;
import bc.bluetooth.SearchServicesTerminatedException;
...
public class BluetoothStackBlueZ implements BluetoothStack {
	private final static String CLSS = "BluetoothStackBlueZ";
	private static Logger LOGGER = Logger.getLogger(CLSS);

	private static BluetoothStackBlueZ instance;
     ...
     */
    public BluetoothStackBlueZ() {
            try {
                System.loadLibrary(BlueCoveConstants.LIBRARY_NAME);
            }
            catch (UnsatisfiedLinkError e) {
                LOGGER.severe(String.format("%s: Native code library %s failed to load (%s)",CLSS,BlueCoveConstants.LIBRARY_NAME,e.getLocalizedMessage()));
        }
    }
    ...
    public void initialize() throws BluetoothStateException {
        ...
        LOGGER.info(String.format("%s.initialize: localDeviceID = %s",CLSS,deviceID));
    ...
    public boolean setLocalDeviceDiscoverable(int mode) throws BluetoothStateException {
        ...
        if (curentMode == mode) {
            ...
            if (error != 0) {
            	LOGGER.severe(String.format("%s.setLocalDeviceDiscoverable:Unable to change discovery mode. It may be because you aren't root (%d)",CLSS,error));
                return false;
            }
            return true;
        }
    ...
    public boolean startInquiry(int accessCode, DiscoveryListener listener) throws BluetoothStateException {
        ...
        DeviceInquiryRunnable inquiryRunnable = new DeviceInquiryRunnable() {
            ...
            public void deviceDiscoveredCallback(DiscoveryListener listener, long deviceAddr, int deviceClass, String deviceName, boolean paired) {
                ...
                LOGGER.info(String.format("%s.deviceDiscoveredCallback:deviceDiscoveredCallback address %s" , CLSS,remoteDevice.getBluetoothAddress()));
                LOGGER.info(String.format("%s.deviceDiscoveredCallback:deviceDiscoveredCallback deviceClass %s" , CLSS,cod));
            ...
            public int runSearchServices(SearchServicesThread sst, int[] attrSet, UUID[] uuidSet, RemoteDevice device, DiscoveryListener listener)
                ...
                try {
                    ...
                    if ((respCode != DiscoveryListener.SERVICE_SEARCH_ERROR) && (sst.isTerminated())) {
                        ...
                        if (records.size() != 0) {
                        	LOGGER.info(String.format("%s.runSearchServices: finished %d", CLSS,sst.getTransID()));
    ...
    private synchronized long getSDPSession() throws ServiceRegistrationException {
        if (this.sdpSesion == 0) {
            ...
            LOGGER.info(String.format("%s.getSDPSession: opened session %s", CLSS,sdpSesion));
        }
        return sdpSesion;
    }
    ...
    private synchronized void unregisterSDPRecord(ServiceRecordImpl serviceRecord) throws ServiceRegistrationException {
        try {
            ...
            if (registeredServicesCount <= 0) {
                ...
                LOGGER.info(String.format("%s.unregisterSDPRecord: session %s", CLSS,sdpSesion));
                long s = sdpSesion;
                sdpSesion = 0;
