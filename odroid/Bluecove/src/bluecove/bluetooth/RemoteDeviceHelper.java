...
import bluecove.bluetooth.WeakVectorFactory.WeakVector;
...
public abstract class RemoteDeviceHelper {
	private final static String CLSS = "RemoteDeviceHelper";
	...
	private static class RemoteDeviceWithExtendedInfo extends RemoteDevice {
		...
		private void addConnection(Object connection) {
			...
			synchronized (connections) {
				...
				LOGGER.info(String.format("%s.addConnection: connection open, now %d connections", CLSS,connections.size() ));
			}
		}
		...
		private void removeConnection(Object connection) {
			...
			synchronized (connections) {
				...
				LOGGER.info(String.format("%s.removeConnection: connection closed, now %d connections", CLSS,connections.size() ));
			}
		}
		...
		void shutdownConnections() {
			...
			for (Enumeration en = c2shutdown.elements(); en.hasMoreElements();) {
				...
				try {
					...
					LOGGER.info(String.format("%s.shutdownConnections: exception %s", CLSS,e.getLocalizedMessage()));
				}
			}
			synchronized (connections) {
		...
		public boolean isAuthenticated() {
			if (!hasConnections()) {
				LOGGER.info(String.format("%s.isAuthenticated: no connections, Authenticated = false", CLSS));
				return false;
			}
	...
	static RemoteDevice createRemoteDevice(BluetoothStack bluetoothStack, long address, String name, boolean paired) {
		...
		if (dev == null) {
			...
			LOGGER.info(String.format("%s.createRemoteDevice: new devices cached %d", CLSS,address));
	...
	private static BluetoothStack getBluetoothStack() throws RuntimeException {
		return BluetoothStackBlueZ.getInstance();
	}
