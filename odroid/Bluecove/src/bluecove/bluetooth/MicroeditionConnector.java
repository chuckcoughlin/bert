...
import bluecove.javax.bluetooth.BluetoothConnectionException;
import bluecove.javax.bluetooth.L2CAPConnection;
import bluecove.javax.bluetooth.SocketConnection;
import bluecove.javax.bluetooth.UUID;
import bluecove.javax.bluetooth.gcf.socket.ServerSocketConnection;
import bluecove.javax.microedition.io.Connection;
import bluecove.javax.microedition.io.ConnectionNotFoundException;
import bluecove.javax.microedition.io.Connector;
import bluecove.javax.microedition.io.InputConnection;
import bluecove.javax.microedition.io.OutputConnection;
...
public abstract class MicroeditionConnector {
	private final static String CLSS = "MicroeditionConnector";
	...
	private static Hashtable<String, Boolean> suportScheme = new Hashtable<>();
	...
	private static Connection openImpl(String name, int mode, boolean timeouts, boolean allowServer) throws IOException {
		...
		LOGGER.info(String.format("%s.openImpl: connecting to %s",CLSS,name));
		...
		if (schemeBluetooth) {
			bluetoothStack = BluetoothStackBlueZ.getInstance();
		}
		...
		if (isServer) {
			...
			if (schemeBluetooth) {
				...
				connectionParams.timeout = BluetoothConnectionParams.DEFAULT_CONNECT_TIMEOUT;
			}
		}
		/*
	...
	private static void validateL2CAPPSM(int channel, String channelAsString) throws IllegalArgumentException {
		...
		if ((channel < BluetoothConsts.L2CAP_PSM_MIN_JSR_82)) {     // There is a flag to disable this if its a problem
