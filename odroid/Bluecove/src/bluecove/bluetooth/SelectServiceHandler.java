...
public class SelectServiceHandler implements DiscoveryListener {
	private final static String CLSS = "SelectServiceHandler";
	...
	private ServiceRecord findServiceOnDevice(UUID uuid, RemoteDevice device) {
		...
		LOGGER.info(String.format("%s.findServiceOnDevice:searchServices on %s",CLSS,device));
		synchronized (serviceSearchCompletedEvent) {
			try {
				serviceSearchCompleted = false;
				agent.searchServices(null, new UUID[] { uuid }, device, this);
			} catch (BluetoothStateException e) {
				LOGGER.info(String.format("%s.findServiceOnDevice:searchServices on %s ERROR(%s)",CLSS,device,e.getLocalizedMessage()));
				return null;
			}
